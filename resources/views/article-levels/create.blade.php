...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
  <div class="mb-6">
    <h1 class="text-2xl font-semibold text-gray-900">Create New Article Level</h1>
  ...
  <div class="bg-white shadow-sm ring-1 ring-gray-200 rounded-xl">
    <div class="p-6">
      <form action="{{ route('article-levels.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <div>
              ...
              <textarea id="description" name="description" rows="3" placeholder="Describe this article level..."
                class="p-2 mt-1 block w-full rounded-md shadow-sm {{ $errors->has('description') ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500' }}">{{ old('description') }}</textarea>
              @error('description')
              <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
              @enderror
            </div>
            ...
            <div>
              ...
              <div class="mt-1 flex gap-2">
                <input type="color" id="color" name="color" value="{{ old('color', '#6c757d') }}"
                  class="h-10 w-16 rounded-md border border-gray-300 cursor-pointer {{ $errors->has('color') ? 'border-red-500' : '' }}">
                <input type="text" id="colorText" value="{{ old('color', '#6c757d') }}" placeholder="#6c757d"
                  pattern="^#[0-9A-Fa-f]{6}$"
          ...
          <div class="space-y-6">
            <div>
              ...
              <input type="number" id="level_order" name="level_order" value="{{ old('level_order') }}" min="1"
